-- =====================================================================
-- 0021 — Catálogo de países: el selector de país deja de ser texto
-- libre sin control y pasa a ser un desplegable respaldado por una
-- tabla de referencia (todos los países, con nombre y prefijo), con
-- una FK desde tenants.pais para que la base de datos solo acepte
-- códigos de ese catálogo. Requiere haber aplicado antes la 0019
-- (que convierte tenants.pais de enum a texto).
-- =====================================================================
...
INSERT INTO paises (codigo, nombre, prefijo) VALUES
  ('ES', 'España', '+34'),
  ('AF', 'Afganistán', '+93'),
  ('AL', 'Albania', '+355'),
  ('DE', 'Alemania', '+49'),
...
-- Por si quedara algún tenant con un valor legado que ya no exista en
-- el catálogo (p. ej. el antiguo "OTRO"), se reconduce a España antes
-- de exigir la referencia, para que la FK no falle al crearla.
UPDATE tenants
SET pais = 'ES'
WHERE pais IS NULL OR pais NOT IN (SELECT codigo FROM paises);

ALTER TABLE tenants
  ADD CONSTRAINT tenants_pais_fkey FOREIGN KEY (pais) REFERENCES paises (codigo);
