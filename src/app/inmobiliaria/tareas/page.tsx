...
export default async function TareasPage() {
  ...
  const [{ data: propietarios }, { data: compradores }, { data: inmuebles }, { data: agentes }] =
    await Promise.all([
  ...
  const nombreAgente = new Map((agentes ?? []).map((a) => [a.id, a.nombre_completo as string]));
  ...
  const kpis = [
    ...
    {
      label: "Asignadas",
      valor: activas.filter((t) => t.asignado_a).length,
      icono: Users,
