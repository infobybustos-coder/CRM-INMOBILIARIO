...
const PLANES: Record<
  TipoPlan,
  { gratis: string[]; pago: string[] }
> = {
  asesor: {
    gratis: [
      `Hasta ${LIMITES_GRATIS.asesor.propietarios} propietarios`,
      `Hasta ${LIMITES_GRATIS.asesor.inmuebles} inmuebles`,
      `Hasta ${LIMITES_GRATIS.asesor.compradores} compradores`,
      "Agenda",
      "Tareas",
      "Dashboard personal",
    ],
  ...
  inmobiliaria: {
    gratis: [
      `Hasta ${LIMITES_GRATIS.inmobiliaria.propietarios} propietarios`,
      `Hasta ${LIMITES_GRATIS.inmobiliaria.inmuebles} inmuebles`,
      `Hasta ${LIMITES_GRATIS.inmobiliaria.compradores} compradores`,
      `${ADMINS_INCLUIDOS_GRATIS} administrador`,
      `${ASESORES_INCLUIDOS_INMOBILIARIA} asesores`,
    ],
    pago: [
      "Propietarios ilimitados",
      "Inmuebles ilimitados",
      "Compradores ilimitados",
      `Hasta ${ADMINS_INCLUIDOS_PAGO} administradores`,
      `Hasta ${ASESORES_INCLUIDOS_INMOBILIARIA} asesores`,
      `Administrador adicional → ${euros(PRECIO_ADMIN_EXTRA)}/mes`,
      `Asesor adicional → ${euros(PRECIO_ASESOR_EXTRA)}/mes`,
    ],
  },
};
            ...
            <div className="space-y-3">
              ...
              <div className="rounded-lg border border-primary bg-primary/5 p-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-semibold">{euros(PRECIO_MENSUAL[tipoPlan])}/mes</span>
                </div>
                <ul className="mt-3 space-y-1.5 text-sm">
                ...
                <p className="mt-3 text-xs text-muted-foreground">
                  La pasarela de pago todavía no está integrada: tu cuenta empezará en Profesional
                  sin ningún cobro real por ahora.
                </p>
                <Button
                  type="button"
                  onClick={() => enviar("pago")}
                ...
                >
                  {pending && enviando === "pago" ? "Creando cuenta..." : "Comenzar ahora"}
                </Button>
              </div>
            </div>
