...
import { Tabla as TablaAgenda, type EventoFila } from "@/components/asesor/agenda/tabla";
import { Tabla as TablaTareas, type TareaFila } from "@/components/asesor/tareas/tabla";
...
export default async function SeguimientoPage({
  ...
  const [
    ...
    supabase
      .from("eventos_agenda")
      .select("id, tipo, fecha_hora, estado, confirmado, entidad_tipo, entidad_id, inmueble_id, comprador_id")
      .eq("usuario_id", usuario.id)
      .order("fecha_hora", { ascending: true }),
    supabase
      .from("tareas")
      .select("id, titulo, descripcion, fecha_vencimiento, estado, prioridad, entidad_tipo, entidad_id, creado_en, completada_en")
  ...
  const agendaItems: AgendaItem[] = (eventos ?? []).map((e) => {
    ...
    return {
      ...
      estado: e.estado,
      tipo: e.tipo,
      href: hrefRelacionado ?? undefined,
    };
  });
  const tareaItems: AgendaItem[] = (tareas ?? [])
    .filter((t) => t.fecha_vencimiento)
    .map((t) => {
      const { hrefRelacionado } = resolverRelacionadoTarea(t);
      return {
        id: t.id,
    ...
    <div className="space-y-6">
      ...
      {(errorEventos || errorTareas) && (
        <div className="rounded-lg border border-red-500/40 bg-red-500/5 p-4 text-sm text-red-600">
          ...
          {errorTareas && <p className="font-medium">No se pudieron cargar las tareas.</p>}
        </div>
      )}
