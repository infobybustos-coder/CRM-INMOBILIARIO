...
function Tarjeta({
  ...
  propietario: PropietarioConAgente;
  ...
  const nombreAgente = agentesPorId.get(propietario.agente_id ?? "");
  const diasContacto = diasDesde(propietario.fecha_ultimo_contacto);
  ...
  return (
    ...
    >
      ...
      {propietario.direccion && (
        <p className="mt-0.5 truncate text-muted-foreground">{propietario.direccion}</p>
      )}
      ...
      <div className="mt-2 flex items-center justify-between border-t pt-1.5">
        ...
        <span className="text-xs text-muted-foreground">
          {diasContacto === null ? "Sin contacto" : diasContacto === 0 ? "Hoy" : `Hace ${diasContacto}d`}
        </span>
      </div>
    </Link>
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex min-h-12 flex-1 flex-col gap-2 overflow-y-auto">
        {propietarios.map((p) => (
          <Tarjeta key={p.id} propietario={p} agentesPorId={agentesPorId} basePath={basePath} />
        ))}
      </div>
    </div>
  );
...
export function Kanban({
  ...
  return (
    <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      ...
      <DragOverlay>
        {activo ? <Tarjeta propietario={activo} agentesPorId={agentesPorId} /> : null}
      </DragOverlay>
    </DndContext>
  );
}
