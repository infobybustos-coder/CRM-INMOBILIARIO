...
function FormularioPanel({
  ...
  useEffect(() => {
    ...
    cargarDocumentosPropietario(propietario.id).then((docs) => {
...
export function PanelPropietario({
  ...
  propietario: PropietarioPanel | null;
  agentes: Agente[];
  tenantId: string;
  onClose: () => void;
}) {
  const abierto = propietario !== null;
